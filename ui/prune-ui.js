...
async function openPruneModal() {
  ...
  el('prune-body').innerHTML = `<div style="padding:40px;text-align:center;color:var(--muted);font-family:var(--mono);font-size:13px;">⟳ Scanning for unused resources…</div>`;
...
function renderPruneBody() {
  ...
  function previewList(items, nameKey) {
    ...
    return shown.map(i => `<div style="font-family:var(--mono);font-size:11px;color:var(--muted);padding:1px 0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${esc(i[nameKey] || i.name)}</div>`).join('')
      + (more > 0 ? `<div style="font-family:var(--mono);font-size:11px;color:var(--muted);font-style:italic;">…and ${more} more</div>` : '');
  }
  ...
  function imageRows(items) {
    ...
    const header = `
      <div style="display:flex;align-items:center;gap:10px;padding:5px 16px;background:var(--bg3);border-bottom:1px solid var(--border2);">
        <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);flex:1.2;">REPOSITORY</span>
        <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);width:60px;">TAG</span>
        <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);width:70px;text-align:right;">SIZE</span>
        <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);width:100px;text-align:right;">CREATED</span>
        <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);width:60px;text-align:right;">STATUS</span>
      </div>`;
    const rows = shown.map(i => `
      <div style="display:flex;align-items:center;gap:10px;padding:6px 16px;border-bottom:1px solid rgba(42,47,74,.25);">
        ...
        <span style="font-family:var(--mono);font-size:11px;color:var(--slate);width:60px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${esc(i.tag || '–')}</span>
        ...
        <span style="font-family:var(--mono);font-size:11px;color:var(--muted);width:100px;text-align:right;white-space:nowrap;">${esc(i.created || '–')}</span>
    ...
    const moreRow = more > 0 ? `<div style="padding:5px 16px;font-family:var(--mono);font-size:11px;color:var(--muted);font-style:italic;">…and ${more} more</div>` : '';
  ...
  el('prune-body').innerHTML = `
    ...
    <div style="padding:8px 16px 4px;background:var(--bg2);border-bottom:1px solid var(--border);">
      <span style="font-size:10px;font-weight:600;letter-spacing:1px;text-transform:uppercase;color:var(--muted);font-family:var(--mono);display:flex;align-items:center;gap:6px;"><i data-lucide="layers" style="width:12px;height:12px;color:var(--lavender);"></i>Images (${images.length})</span>
    </div>
    ${imageRows(images)}` : ''}
    ${networks.length ? `
    <div style="padding:8px 16px 4px;background:var(--bg2);border-bottom:1px solid var(--border);">
      <span style="font-size:10px;font-weight:600;letter-spacing:1px;text-transform:uppercase;color:var(--muted);font-family:var(--mono);display:flex;align-items:center;gap:6px;"><i data-lucide="cable" style="width:12px;height:12px;color:var(--coral);"></i>Networks (${networks.length})</span>
    </div>
    ${networkRows(networks)}` : ''}
    ${volumes.length ? `
    <div style="padding:8px 16px 4px;background:var(--bg2);border-bottom:1px solid var(--border);">
      <span style="font-size:10px;font-weight:600;letter-spacing:1px;text-transform:uppercase;color:var(--muted);font-family:var(--mono);display:flex;align-items:center;gap:6px;"><i data-lucide="hard-drive" style="width:12px;height:12px;color:var(--yellow);"></i>Volumes (${volumes.length})</span>
    </div>
    ${volumeRows(volumes)}` : ''}
    ${buildCache.length ? `
    <div style="padding:8px 16px 4px;background:var(--bg2);border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;">
      <span style="font-size:10px;font-weight:600;letter-spacing:1px;text-transform:uppercase;color:var(--muted);font-family:var(--mono);display:flex;align-items:center;gap:6px;"><i data-lucide="hammer" style="width:12px;height:12px;color:var(--orange);"></i>Build Cache (${buildCache.length})</span>
      ${buildCacheTotal ? `<span style="font-family:var(--mono);font-size:11px;color:var(--muted);">Total: <span style="color:var(--accent);">${esc(buildCacheTotal)}</span>${buildCacheReclaimable ? `  ·  Reclaimable: <span style="color:var(--green);">${esc(buildCacheReclaimable)}</span>` : ''}</span>` : ''}
    </div>
...
function networkRows(items) {
  ...
  const header = `
    <div style="display:flex;align-items:center;gap:10px;padding:5px 16px;background:var(--bg3);border-bottom:1px solid var(--border2);">
      <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);flex:1;">NAME</span>
      <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);width:80px;">DRIVER</span>
      <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);width:60px;">SCOPE</span>
    </div>`;
  const rows = shown.map(n => `
    <div style="display:flex;align-items:center;gap:10px;padding:6px 16px;border-bottom:1px solid rgba(42,47,74,.25);">
      ...
      <span style="font-family:var(--mono);font-size:11px;color:var(--slate);width:80px;white-space:nowrap;">${esc(n.driver || '–')}</span>
      <span style="font-family:var(--mono);font-size:11px;color:var(--muted);width:60px;white-space:nowrap;">${esc(n.scope || '–')}</span>
    </div>`).join('');
  const moreRow = more > 0 ? `<div style="padding:5px 16px;font-family:var(--mono);font-size:11px;color:var(--muted);font-style:italic;">…and ${more} more</div>` : '';
  return header + rows + moreRow;
}
...
function volumeRows(items) {
  ...
  const header = `
    <div style="display:flex;align-items:center;gap:10px;padding:5px 16px;background:var(--bg3);border-bottom:1px solid var(--border2);">
      <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);flex:1;">NAME</span>
      <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);width:60px;">DRIVER</span>
      <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);width:90px;text-align:right;">CREATED</span>
      <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);flex:1.5;padding-left:10px;">MOUNT PATH</span>
    </div>`;
  const rows = shown.map(v => `
    <div style="display:flex;align-items:center;gap:10px;padding:6px 16px;border-bottom:1px solid rgba(42,47,74,.25);">
      <span style="font-family:var(--mono);font-size:12px;color:var(--text);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"
        title="${esc(v.name)}">${esc(v.name)}</span>
      <span style="font-family:var(--mono);font-size:11px;color:var(--slate);width:60px;white-space:nowrap;">${esc(v.driver || '–')}</span>
      ...
      <span style="font-family:var(--mono);font-size:11px;color:var(--muted);flex:1.5;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding-left:10px;"
...
function buildCacheRows(items) {
  ...
  const header = `
    <div style="display:flex;align-items:center;gap:10px;padding:5px 16px;background:var(--bg3);border-bottom:1px solid var(--border2);">
      <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);flex:1;">ID</span>
      <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);width:90px;text-align:right;">RECLAIMABLE</span>
      <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);width:70px;text-align:right;">SIZE</span>
      <span style="font-family:var(--mono);font-size:10px;font-weight:600;letter-spacing:.8px;color:var(--muted);width:130px;text-align:right;">LAST ACCESSED</span>
    </div>`;
  const rows = shown.map(c => `
    <div style="display:flex;align-items:center;gap:10px;padding:6px 16px;border-bottom:1px solid rgba(42,47,74,.25);">
      ...
      <span style="font-family:var(--mono);font-size:11px;color:var(--muted);width:130px;text-align:right;white-space:nowrap;">${esc(c.lastAccessed || '–')}</span>
    </div>`).join('');
  const moreRow = more > 0 ? `<div style="padding:5px 16px;font-family:var(--mono);font-size:11px;color:var(--muted);font-style:italic;">…and ${more} more</div>` : '';
  return header + rows + moreRow;
}
...
async function openPruneLogModal() {
  ...
  try {
    ...
    if (!lines.length) {
      el('prune-log-body').innerHTML = `<span style="color:var(--muted);">No prune activity yet.</span>`;
      return;
    }
