...
async function handleHomepageWidget(req, res) {
	...
	const { processes = [] } = _getCache();
	const containerPids = new Set(running.flatMap(c => c.pids || []));
	const containerProcs = processes.filter(p => containerPids.has(p.pid));
	const diskReadKBs  = containerProcs.reduce((sum, p) => sum + (p.diskReadKBs  || 0), 0);
	const diskWriteKBs = containerProcs.reduce((sum, p) => sum + (p.diskWriteKBs || 0), 0);
	...
	jsonOk(res, {
		containers: {
			...
			net_total_kbs: parseFloat((netRxKBs + netTxKBs).toFixed(2)),
			disk_read_kbs: parseFloat(diskReadKBs.toFixed(2)),
			disk_write_kbs: parseFloat(diskWriteKBs.toFixed(2)),
			disk_io_kbs: parseFloat((diskReadKBs + diskWriteKBs).toFixed(2)),
		},
	});
}

// ─── /api/homepage/containers ────────────────────────────────────────────────

async function handleHomepageContainers(req, res) {
	const { containers, processes = [] } = _getCache();
	const running = containers.filter(c => c.state === 'running');
	const sorted = [...running].sort((a, b) => (parseFloat(b.cpu) || 0) - (parseFloat(a.cpu) || 0));
	const top = sorted.slice(0, 10);

	const pidDiskMap = new Map(processes.map(p => [p.pid, (p.diskReadKBs || 0) + (p.diskWriteKBs || 0)]));

	const rows = top.map(c => {
		const nwKBs   = (c.vethRxKBs || 0) + (c.vethTxKBs || 0);
		const diskKBs = (c.pids || []).reduce((sum, pid) => sum + (pidDiskMap.get(pid) || 0), 0);
		return {
			name: c.name,
			usage: `${(parseFloat(c.cpu) || 0).toFixed(1)}% · ${(parseFormattedBytes(c.memUsage) / 1024 ** 3).toFixed(1)} GB · ${nwKBs.toFixed(1)} KB/s · ${diskKBs.toFixed(1)} KB/s`,
		};
	});

	jsonOk(res, {
		containers: [{ name: 'Container', usage: 'CPU · RAM · Network · Disk' }, ...rows],
	});
}
...
module.exports = { setDependencies, handleApi, handleHomepageWidget, handleHomepageContainers };
